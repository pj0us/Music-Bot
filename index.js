...
const { TOKEN, prf } = require(`./config.json`);
let PREFIX
...
client.on(`ready`, () => {	
   ...
   ///////////////////////////////
    ...
    figlet.text(`${client.user.username} ready!`, function (err, data) {
      ...
      ///////////////////////////////////////
let mem = 0
client.guilds.cache.map(guild => mem = mem + guild.memberCount)
      const activities = [
...
client.on(`message`, async (message) => {
  ...
  if(message.content === (`<@!864821086490066974>`)) {
    message.reply(new Discord.MessageEmbed().setColor("#ffe700").setAuthor(`${message.author.username}, My Prefix is ${PREFIX} to get started type ${PREFIX}help`, message.author.displayAvatarURL({dynamic:true})));
  } 
 ...
 if (timestamps.has(message.author.id)) {
   ...
   if (now < expirationTime) {
     ...
     return message.reply(
      new MessageEmbed().setColor("#ffe700")
