...
async execute(message, args, client) {
    ...
    //If no args return
    if (!args.length)
      return attentionembed(message, `Usage: ${prf}play <YouTube URL | Video Name | Soundcloud URL>`);
    ...
    if (urlValid) {
      try {
        ...
        song = {
          ...
          request: `${message.member.user.username}#${message.member.user.discriminator}`,
       };
      } catch (error) {
