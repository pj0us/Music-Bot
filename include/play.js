...
module.exports = {
  async play(song, message, client, filters) {
    ...
    if (!song) {
      ...
      const endembed = new MessageEmbed().setColor("#ffe700")
        .setAuthor(`Music Queue ended so i left the voice channel.`)
    ...
    try {
      if (song.url.includes("youtube.com")) {
        stream = await ytdl_dis(song.url, {
          filter: "audioonly",
          opusEncoded: true,
    ...
    collector.on("collect", async (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        case "⏭":
          ...
          const skipembed = new MessageEmbed().setColor("#ffe700").setAuthor(`${user.username} skipped the song.`, "https://images.emojiterra.com/twitter/v13.0/128px/23ed.png")
        ...
        case "⏯":
          ...
          if (queue.playing) {
            ...
            const pausemebed = new MessageEmbed().setColor("#ffe700")
              .setAuthor(`${user.username} paused the music.`, "https://hotemoji.com/images/dl/1/play-or-pause-button-emoji-by-twitter.png")
            queue.textChannel.send(pausemebed).catch(console.error);
          } else {
            queue.playing = !queue.playing;
            queue.connection.dispatcher.resume();
            const playembed = new MessageEmbed().setColor("#ffe700")
              .setAuthor(`${user.username} resumed the music!`, "https://hotemoji.com/images/dl/1/play-or-pause-button-emoji-by-twitter.png")
        ...
        case "🔄":
          ...
          const loopembed = new MessageEmbed().setColor("#ffe700")
            .setAuthor(`Loop is now ${queue.loop ? "enabled" : "disabled"}`, "https://cdn-0.emojis.wiki/emoji-pics/twitter/repeat-button-twitter.png")
        ...
        case "⏹":
          ...
          if (!canModifyQueue(member)) return;
          queue.songs = [];
          const stopembed = new MessageEmbed().setColor("#ffe700").setAuthor(`${user.username} stopped the music!`, "https://cdn-0.emojis.wiki/emoji-pics/twitter/stop-button-twitter.png")
